<?php
/**
 * ACF_Field_Position File Doc Comment
 *
 * @package acf-position
 * @author Mikhail Kozarev
 * @license  http://www.gnu.org/copyleft/gpl.html GNU General Public License
 */

/**
 * Class ACF_Field_Position
 */
class ACF_Field_Position extends acf_field {

	/**
	 * Страна/Регион/Город по дефолту
	 *
	 * @var array
	 */
	private $defaults;
	...
	function _acf_get_countries() {
		...
		$countries_db = $wpdb->get_results(
			"SELECT * 
			 ...
			 ORDER BY ( 'Россия' != country_name ), country_name ASC"
	...
	function _acf_get_cities( $region_id ) {
		global $wpdb;
		$cities_db = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * 
				 FROM {$wpdb->prefix}cities 
				 WHERE region_id=%d 
				 ORDER BY city_name ASC",  $region_id
			)
		); // db call ok; db cache ok.
		...
		foreach ( $cities_db as $city ) {
			...
			$cities[ $city->city_id ] = ( ! empty( $city->area_name ) ) ?
				sprintf( '%s (%s)', $city->city_name, $city->area_name ) :
				$city->city_name;
		}
	...
	function _acf_get_regions( $country_id ) {
		global $wpdb;
		$regions_db = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * 
				 FROM {$wpdb->prefix}regions 
				 WHERE country_id = %d 
				 ORDER BY region_name ASC", $country_id
			)
		); // db call ok; db cache ok.
	...
	function render_field( $field ) {
		...
		// TODO: (.acf-label > label ) + ( .acf-input > input ).
		?>

		<ul class="country-selector-list">
			<li id="field-<?php echo esc_attr( $key ); ?>[country_id]">
				<div class="field-inner">
					<strong><?php esc_html_e( 'Страна', 'acf' ); ?></strong><br />
			...
			<li id="field-<?php echo esc_attr( $key ); ?>[region_id]">
				<div class="css3-loader" style="display:none;"><div class="css3-spinner"></div></div>
				<div class="field-inner">
					<strong><?php esc_html_e( 'Регион', 'acf' ); ?></strong><br />
...
// create field
$GLOBALS['acf_position'] = new ACF_Field_Position();
